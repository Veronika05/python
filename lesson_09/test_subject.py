...
def test_insert():
    ...
    result = connection.execute(text("SELECT * FROM subject where subject_title = :title"), {"title" : 'Предмет для тестирования'} ) 
    rows = result.mappings().all()
    assert len(rows) > 1
...
def test_update():
    ...
    sql = text("update subject set subject_title = 'updated' where subject_title = :title")
    connection.execute(sql, {"title":'Новый предмет'})
    result = connection.execute(text("SELECT * FROM subject where subject_title = :title"), {"title" : 'updated'} ) 
    rows = result.mappings().all()
    assert len(rows) ==  1
    sql = text("DELETE FROM subject WHERE subject_title = :title")
...
def test_delete():
    ...
    result = connection.execute(text("SELECT * FROM subject where subject_title = :title"), {"title" : 'Предмет для удаления'} ) 
    rows = result.mappings().all()
    assert len(rows)  == 1
    sql = text("DELETE FROM subject WHERE subject_title = :title")
